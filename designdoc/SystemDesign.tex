...
As shown in Figure~\ref{fig:eepromFrmt}, the keys are updated on EEPROM at system build, and other data such as Tags, IVs are updated at run-time.
\begin{figure}[!htbp]
	\begin{centering}
		\includegraphics[width = .98\textwidth]{pic/EEPROM_format.pdf}
		\caption{EEPROM Key and other data storage format}
		\label{fig:eepromFrmt}
	\end{centering}
\end{figure}
...
Figure~\ref{fig:eepromFrmt} shows the storage format of those data on EEPROM memory.
